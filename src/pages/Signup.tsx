...
const Signup = () => {
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 rounded border border-gray-200">
      <h1 className="text-xl font-semibold mb-4 text-black">Sign Up</h1>
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input type="password" className={inputClass} placeholder="Choose a secure password" value={password} onChange={(e) => setPassword(e.target.value)} />
        </div>
        {error && <p className="text-red-600 text-sm">{error}</p>}
        <button type="submit" disabled={submitting} className={btnClass}>{submitting ? 'Please wait...' : 'Create account'}</button>
      </form>
      <div className="mt-4 text-sm text-gray-800">
        <p>
          Already have an account? <Link className="text-[#6B4C3B] underline" to="/login">Login</Link>
        </p>
      </div>
    </div>
  )
